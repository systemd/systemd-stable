...
TEST(basic_mask_and_enable) {
        ...
        assert_se(unit_file_get_state(RUNTIME_SCOPE_SYSTEM, root, "g.service", NULL) == -ENOENT);
        assert_se(unit_file_get_state(RUNTIME_SCOPE_SYSTEM, root, "h.service", NULL) == -ENOENT);
        ...
        assert_se(unit_file_enable(RUNTIME_SCOPE_SYSTEM, 0, root, STRV_MAKE("g.service"), &changes, &n_changes) >= 0);
        install_changes_free(changes, n_changes);
        changes = NULL; n_changes = 0;

        assert_se(unit_file_get_state(RUNTIME_SCOPE_SYSTEM, root, "g.service", &state) >= 0);
        assert_se(state == UNIT_FILE_ENABLED);

        assert_se(unit_file_get_state(RUNTIME_SCOPE_SYSTEM, root, "h.service", &state) >= 0);
        assert_se(state == UNIT_FILE_ALIAS);
}
